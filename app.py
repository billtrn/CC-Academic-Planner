...
def calendar():
    ...
    form = CourseForm()
    if form.department.data is None:
    ...
    if request.method == "POST":
        if form.validate_on_submit():
            ...
            # if there are one or more conflicts with the current class schedule, do not add it and report conflict
                ...
                error = "The selected class, " + course["title"] + " (" + "".join(course["days"]) + ", " + "".join(
                    course["times"]) + ")," + " conflicts with the following courses on your schedule: " + conlist
        ...
        if form_remove.submit.data:
            for s in form_remove.selcourses.data:
                for i in cs:
                    if i["crn"] == s:
                        cs.remove(i)
                        form_remove.selcourses.choices.remove((s, (i["title"] + " " + i["sect"])))
                        credits -= data.getCredits(i["crn"])
